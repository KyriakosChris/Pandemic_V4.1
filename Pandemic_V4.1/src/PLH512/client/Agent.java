...
public class Agent {
	...
	/*
	 * 1) NORMALIZATION OF DISTANCE TO ALL THE CITIES USING THE NUMBER OF CITY CUBES
	 * DIVIED BY TOTAL STATE CUBES
	 */
	...
	/*
	 * 3) MIN NUMBER OF CARDS MISSING TO DISCOVER A CURE FOR EACH DISEASE COLOR
	 * AMONG PLAYERS' HANDS
	 */
	...
	/*
	 * 4) NUMBER OF DISCARDED CARDS FOR EACH OF THE ACTIVE DISEASES STILL MISSING A
	 * CURE
	 */
	...
	/*
	 * 6) AVERAGE DISTANCE REQUIRED TO MOVE FROM EACH CITY TO ANOTHER ASSOSIATED
	...
	public static double heuristicDistance(Board board) {
		...
		for (int i = 0; i < board.getCitiesCount(); i++) {
			...
			for (int j = 0; j < board.getCitiesCount(); j++) {
				dist += (Client.distanceFrom(Cities.get(j).getName(), distanceMap) / constant)
						* (cardsPlayedSoFar / (totalCityCards + epidemicCards));
			}
		}
		return dist;
	}
	...
	public void setTotalTreats(int totalTreats) {
